...
def import_data(filename):
    ...
    with open(filename, 'r') as testfile:
        filereader = csv.reader(testfile)
        for line in filereader:
            pokemonstats = [line[0], line[1], float(line[2]), float(line[3])]
            participants.append(pokemonstats)
...
def fight(participant1, participant2, first2attack):
    ...
    while HP1 > 0 and HP2 > 0:
        if first2attack == 1:
            attackbonus = attack_multiplier(participant1[1], participant2[1])
            newdamage = attackbonus * participant1[3]
            HP2 = HP2 - newdamage
        else:
            attackbonus = attack_multiplier(participant2[1], participant1[1])
            newdamage = attackbonus * participant2[3]
            HP1 = HP1 - newdamage
        rounds += 1
        if first2attack == 1:
            first2attack = 2
...
def tournament(participants):
    ...
    ######################
    wins = []
    while len(wins) < len(participants):
        wins.append(0)
