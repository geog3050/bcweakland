...
#Get mode input:
while True:
    mode = input('Enter your desired mode (min, max, or avg):')
    if mode in ['min', 'max', 'avg']:
        break
    else:
...
#Get count input:
while True:
    countString = input('Enter the number of values in your desired list:')
    try:
...
i = 1
while i <= count:
    valueInput = input('Please input a value in your desired list:')
    try:
...
#Determine minimum, maximum, or average of list:
if mode == 'min':
    minimum = floatList[0]
